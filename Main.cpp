...
int main (int argc, char* argv[]) {
    ...
    string pathC = "./csv-exemplos/csv/ES/vitoria/candidatos.csv";
    string pathP = "./csv-exemplos/csv/ES/vitoria/partidos.csv";
    string data = "15/11/2020";
    ...
    fileC.open (pathC);
    if(!fileC.is_open()) {
        cout<< "Não foi possível abrir o arquivo de Candidatos." << endl;
        return 0;
    }

    fileP.open (pathP);
    if(!fileP.is_open()){ 
        cout<< "Não foi possível abrir o arquivo de Partidos."<< endl;
        return 0;
    }
    ...
    for(Partido p: partidos)
         if(p.getVotosLegenda()>0)
             relatorio.printPartidoColocacoesCandidatos(relatorio.getIndexPartido(partidos,p)+1, p);		
